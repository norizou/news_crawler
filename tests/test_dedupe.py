...
class TestNormalizeTitleForDedupe:
    ...
    def test_full_width_space(self):
        assert (
            normalize_title_for_dedupe("大井競馬　ゴールドジュニア") == "大井競馬ゴールドジュニア"
        )

    def test_nfkc_normalizes_width(self):
        # Full-width alphanumerics collapse to half-width under NFKC.
        assert normalize_title_for_dedupe("Ｖ３重賞初制覇") == normalize_title_for_dedupe(
            "V3重賞初制覇"
        )
...
class TestFindDuplicateMatches:
    def test_real_duplicate_pair_detected(self):
        ...
        title = (
            "【大井競馬　ゴールドジュニア】牝馬パープルフォッグ　無傷V3重賞初制覇　最後まで余力あり"
        )
        earlier = make_article(
            "netkeiba",
            "https://netkeiba.example/1",
            title,
            datetime(2026, 9, 17, 21, 26, 39, tzinfo=timezone(timedelta(hours=9))),
        )
        later = make_article(
            "sponichi",
            "https://sponichi.example/1",
            title,
    ...
    def test_same_source_not_matched(self):
        ...
        title = (
            "中山馬主協会の西川賢会長が熊谷千葉県知事を訪問　"
            "豪雨被害の「千葉県を助けよう」という気持ちで"
        )
    ...
    def test_short_title_below_threshold_ignored(self):
        ...
        a = make_article(
            "netkeiba", "https://netkeiba.example/1", "重賞見どころ", datetime(2026, 9, 17)
        )
        b = make_article(
            "sponichi", "https://sponichi.example/1", "重賞見どころ", datetime(2026, 9, 18)
        )
        a.id, b.id = 1, 2
...
class TestRunDedupe:
    def test_persists_and_is_idempotent(self, temp_db: Database, sources: list[SourceConfig]):
        ...
        canonical, _, _ = temp_db.upsert_article(
            make_article(
                "netkeiba", "https://netkeiba.example/1", title, datetime(2026, 9, 17, 21, 0)
            )
        )
        duplicate, _, _ = temp_db.upsert_article(
            make_article(
                "sponichi", "https://sponichi.example/1", title, datetime(2026, 9, 18, 1, 0)
            )
        )
